  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Başarılı",
          description: `${formData.name} markası başarıyla eklendi.`,
          variant: "success",
        })
        router.push('/admin/brands')
      } else {
        throw new Error(data.error || 'Marka oluşturulurken hata oluştu')
      }
    } catch (error: any) {
      setError(error.message || 'Sunucu hatası')
      toast({
        title: "Hata",
        description: error.message || 'Marka eklenirken hata oluştu',
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-2">Yeni bir marka oluşturun</p>
        </div>
      </div>
      ...
      <Card className="max-w-2xl">
        <CardHeader>
          ...
          <CardDescription>
            Markanızın tüm detaylarını girin
          </CardDescription>
        </CardHeader>
        <CardContent>
